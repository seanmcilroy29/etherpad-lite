...
const lookupEnvironmentVariables = (obj) => {
  const stringifiedAndReplaced = JSON.stringify(obj, (key, value) => {
    ...
    if ((envVarValue === undefined) && (defaultValue === undefined)) {
      console.warn(`Environment variable "${envVarName}" does not contain ` +
      `any value for configuration key "${key}", and no default was given. Returning null.`);
     ...
     */
    console.debug(`Configuration key "${key}" will be read ` +
...
exports.reloadSettings = () => {
  ...
  if (!exports.skinName) {
    console.warn('No "skinName" parameter found. Please check out ' +
    'settings.json.template and update your settings.json. ' +
  ...
  if (exports.skinName) {
    ...
    if (countPieces !== 1) {
      ...
      `This is not valid: "${exports.skinName}". Falling back to the default "colibris".`);
      ...
      fs.exists(exports.abiword, (exists) => {
        if (!exists) {
          ...
          if (!exports.suppressErrorsInPadText) {
            ...
            `${abiwordError}${suppressDisableMsg}`;
          }
  ...
  if (exports.soffice) {
    fs.exists(exports.soffice, (exists) => {
      if (!exists) {
        const sofficeError = 'soffice (libreoffice) does not exist at this ' +
        'path, check your settings file.';

        if (!exports.suppressErrorsInPadText) {
          exports.defaultPadText = `${exports.defaultPadText}\nError:` +
          ` ${sofficeError}${suppressDisableMsg}`;
        }
  ...
  if (!exports.sessionKey) {
    ...
    try {
      ...
      console.info(`Session key file "${sessionkeyFilename}" ` +
        'not found. Creating with random contents.');
  ...
  if (exports.ip === '') {
    ...
    console.warn('The settings file contains an empty string ("") for the "ip" ' +
    'parameter. The "port" parameter will be interpreted as the path to a Unix socket to bind at.');
  }
