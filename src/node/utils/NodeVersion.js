...
exports.checkDeprecationStatus = (lowestNonDeprecatedNodeVersion, epRemovalVersion) => {
  ...
  if (semver.lt(currentNodeVersion, lowestNonDeprecatedNodeVersion)) {
    console.warn(`Support for Node ${currentNodeVersion} will be removed in Etherpad` +
      ` ${epRemovalVersion}. Please consider updating at least to ` +
      `Node ${lowestNonDeprecatedNodeVersion}`);
  }
};
